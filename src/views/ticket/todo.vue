...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleFilter(){
        this.$store.dispatch('get_ticket_list', {category: 'duty', is_end: '0', sn: this.search_data.ticket_id, workflow_name: this.search_data.workflow_name, page: this.page, per_page: this.limit}).then(resp => {
          this.tableData = resp.data.data.value
          this.total = resp.data.data.total
        })
      },
      init(){
        this.$store.dispatch('get_ticket_list', {category: 'duty', is_end: '0'}).then(resp => {
          this.tableData = resp.data.data.value
          this.total = resp.data.data.total
        }).catch(err => {
          console.log(err)
      ...
      getList(){
        this.$store.dispatch('get_ticket_list', {category: 'duty', is_end: '0',page: this.page, per_page: this.limit}).then(resp => {
          this.tableData = resp.data.data.value
          this.total = resp.data.data.total
        }).catch(err => {
          console.log(err)
