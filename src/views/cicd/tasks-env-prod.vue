...
-->
        ...
        <el-card class="box-card" style="width: 240px;display: inline-block;margin-right: 5px" v-for="(item,index) in task_list" :key="index">
          <div slot="header" class="clearfix">
            ...
            <!-- <el-button type="warning" icon="el-icon-s-order" circle size="mini" style="float: right" title="提交审批"
                       @click="approvalOpen({task_id: item.id})" v-if="item.ready && !item.finish && !item.deleted && item.is_approval===1" :disabled="item.deleted"></el-button> -->
            <el-button type="primary" icon="el-icon-s-order" circle size="mini" style="float: right" title="提交审批"
                       @click="ApprovalTask(item)" v-if="item.ready && !item.finish && !item.deleted && item.is_approval===0" :disabled="item.deleted"></el-button>

            <el-button type="warning" icon="el-icon-s-order" circle size="mini" style="float: right" title="审批中"
                      @click="approvalStatus(2)" v-if="item.ready && !item.finish && !item.deleted && item.is_approval===1" :disabled="item.deleted"></el-button>
             <el-button type="danger" icon="el-icon-s-order" circle size="mini" style="float: right" title="审批已拒绝"
                      @click="approvalStatus(3)" v-if="item.ready && !item.finish && !item.deleted && item.is_approval===2" :disabled="item.deleted"></el-button>
            <el-button type="success" icon="el-icon-video-play" circle size="mini" style="float: right" title="开始"
                       @click="executeTask({task_id: item.id})" v-if="item.ready && !item.finish && !item.deleted && item.is_approval===3" :disabled="item.deleted"></el-button>
...
<!--              <el-table-column label="镜像" prop="image" :show-overflow-tooltip="true"></el-table-column>-->
      ...
      <el-dialog :title="newTitle" :visible.sync="dialogEditTaskForm" :close-on-click-modal="false" width="80%" @close="closeTaskEdit">
        <el-form size="small" label-width="120px" :rules="newRules" :model="taskEditRow" ref="taskEditRow" label-position="right">
          ...
          <el-form-item label="异常处理" prop="exception" :rules="[{required:true,message:'请选择异常处理方式.', trigger: 'blur'}]">
            <div><el-radio v-model="taskEditRow.exception" label="stop" :disabled="is_approval_start">执行过程中有发布单元异常，则发布中断（默认）</el-radio></div>
            <!--            <div><el-radio v-model="taskEditRow.exception" label="rollback">执行过程中有发布单元异常，则发布任务整体回退至发布前状态</el-radio></div>-->
            <div><el-radio v-model="taskEditRow.exception" label="continue" :disabled="is_approval_start">执行过程中有发布单元异常，发布继续执行</el-radio></div>
          </el-form-item>
          <el-form-item label="发布单元">
            <el-button icon="el-icon-plus" @click="showTaskUnit" style="margin-bottom: 5px" :disabled="is_approval_start">添加</el-button>
            ...
            <el-table border :data="edit_unit_list" size="mini" highlight-current-row ref="singleTaskUnit" class="task-unit" :row-key="rowKey" :default-sort="{prop: 'order_id'}">
              ...
              <el-table-column label="操作" width="220" align="center">
                <template slot-scope="scope">
                  <Button loading v-if="scope.row.ready===false && scope.row.finish===false"></Button>
                  <Button type="success" icon="md-play" title="发布" @click="executeTask({task_id: taskEditRow.id, release_id: scope.row.id})"
                          :disabled="scope.row.finish || !taskEditRow.id || scope.row.rollback || !scope.row.id" v-if="scope.row.ready && !taskEditRow.deleted && taskEditRow.is_approval===3"></Button>
                  ...
                  <Button type="error" icon="ios-trash-outline" title="删除" @click="confirmDeleteUnit(scope.row)" :disabled="scope.row.finish || !scope.row.ready || scope.row.rollback || taskEditRow.is_approval!==0"></Button>
                </template>
              </el-table-column>
            </el-table>
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button type="warning" plain="plain" @click="UpdateTask('PUT')" v-if="taskEditRow.id && !taskEditRow.finish && !is_deployed && !taskEditRow.deleted" :disabled="!taskEditRow.ready || is_approval_start">更 新</el-button>
...
<!--          <el-button type="primary" plain="plain" @click="showCreateAppVersion">创建新版本</el-button>-->
      ...
      <!-- 提交审批 -->
      <el-dialog title="审批内容" :visible.sync="dialogApprovalTask" :close-on-click-modal="false" width="70%" @close="closeTaskEdit">
        <el-form size="small" label-width="120px" :rules="newRules" :model="taskEditRow" ref="taskEditRow" label-position="right">
          <el-form-item hidden>
            <el-input v-model="taskEditRow.id" style="width: 240px"></el-input>
          </el-form-item>
          <el-form-item label="任务名" prop="name" >
            <el-input autocomplete="off" v-model.trim="taskEditRow.name" style="width: 240px" :disabled="true"></el-input>
          </el-form-item>
          <el-form-item label="异常处理" prop="exception" :rules="[{required:true,message:'请选择异常处理方式.', trigger: 'blur'}]">
            <div><el-radio v-model="taskEditRow.exception" label="stop" :disabled="true">执行过程中有发布单元异常，则发布中断（默认）</el-radio></div>
            <!--            <div><el-radio v-model="taskEditRow.exception" label="rollback">执行过程中有发布单元异常，则发布任务整体回退至发布前状态</el-radio></div>-->
            <div><el-radio v-model="taskEditRow.exception" label="continue" :disabled="true">执行过程中有发布单元异常，发布继续执行</el-radio></div>
          </el-form-item>
          <el-form-item label="发布单元">
          ...
          <el-form-item>
          <div>审批详情:<a href="https://taskscenter-test.oyohotels.cn/" target="_blank"> https://taskscenter-test.oyohotels.cn/</a></div>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button type="warning" plain="plain" @click="approvalOpen({task_id: taskEditRow.id})" v-if="taskEditRow.id && !taskEditRow.finish && !is_deployed && !taskEditRow.deleted" :disabled="!taskEditRow.ready">提交审批</el-button>
          <el-button @click="dialogApprovalTask = false">取 消</el-button>
        </div>
      </el-dialog >
...
<script>
  ...
  export default {
    ...
    data(){return{
      ...
      taskEditRow: {id: '', name: '', 'exception': 'stop', unit_list: [], ready: true, finish: false, is_approval: ''},
      dialognewTaskForm: false,
      dialogEditTaskForm: false,
      dialognewTaskUnit: false,
      dialogAppVersion: false,
      dialogApprovalTask: false,
      ...
      task_unit_timer: '',
      deploy_timer: '',
      approval_list : [{id:1, 'name':'江双','email':'victor.jiang@oyohotels.cn'},{id:2, 'name':'江哈', 'email':'jiang@oyohotels.cn'}],
      is_approval_start: false,
    }},
    watch: {
    ...
    methods: {
      ...
      showEditTask: function(row){
        ...
        this.taskEditRow.is_approval = row.is_approval
        this.newTitle = '编辑任务'
        this.getTaskUnit()
        this.dialogEditTaskForm = true
        this.disabled_task_name = true
        if (this.taskEditRow.is_approval == 0) {
          this.is_approval_start = false
        } else {
          this.is_approval_start = true
        }
        console.log(this.is_approval_start)
      },
      ...
      getTaskUnitCiLog: function(){
        // console.log("params",  this.select_unit.app_version)
        ...
        this.axios({
          ...
          // console.log('getTaskUnitCiLog:',response.data);
          if(response.data.status){
            if(response.data.data.length>0){
      ...
      getTaskData: function(){
        let params = {"pageSize": this.task_limit, "pageNumber": this.task_page, deleted: this.formSearch.deleted,
          name: this.formSearch.name,'finish': this.formSearch.task_status, 'env': 'prod'};
        this.axios({
          method: 'GET',
          url: this.$store.state.cicd.task_url,
          params: params,
      ...
      saveTask: function(method){
        this.$refs.newRow.validate(valid =>{
          if(valid){
            let parms = {'name': this.newRow.name, 'strategy': this.newRow.exception, 'unit_list': this.unit_list, 'env': 'prod'}
      ...
      UpdateTask: function(method){
        this.$refs.taskEditRow.validate(valid =>{
          if(valid){
            let parms = {'name': this.taskEditRow.name, 'strategy': this.taskEditRow.exception, 'unit_list': this.edit_unit_list, 'env': 'prod'}
      ...
      GetVersionStatus: function() {
        this.$store.dispatch('GetVersionStatus', {'app_version_id': this.cur_version.id}).then(response=>{
          // console.log('GetVersionStatus:',response.data);
          if(response.data.status){
      ...
      envChange: function(){
        if(this.deployment_env==='prod'){
          ...
          if(this.select_app_version.uat!=='success'){
            this.deployment_env=''
            this.$message.warning('当前版本尚未发布过UAT环境或发布UAT失败。')
            return
          }else if(this.select_app_version.uat==='success') {
            this.GetAppCiCdKube()
      ...
      // 显示审批提交内容
      ApprovalTask: function(row){
        this.taskEditRow.id = row.id
        this.getTaskUnit()
        this.dialogApprovalTask = true
      },
     ...
     approvalOpen(params) {
        this.$confirm('提交审批至审批平台, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      approvalStatus(params) {
        if (params == 2) {
          this.$alert('任务正在审批中 <div> <br> 审批详情:<a href="http://taskscenter.dev.ahotels.tech" target="_blank"> http://taskscenter.dev.ahotels.tech</a></div>', '提示', {
            dangerouslyUseHTMLString: true
          }
          )
        } else if (params == 3) {
          this.$alert('任务审批已拒绝 <div> <br> 审批详情:<a href="http://taskscenter.dev.ahotels.tech" target="_blank"> http://taskscenter.dev.ahotels.tech</a></div>', '提示', {
            dangerouslyUseHTMLString: true
        })
        }
      }
