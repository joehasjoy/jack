<template>
    ...
    <el-card style="margin: 10px">
      ...
      <el-table id="out-table" v-loading="$store.state.kube.loading"
        ...
        <el-table-column sortable label="类型" prop="type" width="100"></el-table-column>
        <el-table-column sortable label="clusterIP" prop="cluster_ip" width="150"></el-table-column>
        <el-table-column sortable label="内部端点" prop="">
          <template slot-scope="scope">
            <div v-for="(item, key) in scope.row.detail.spec.ports" :key="key">
              <template v-if="scope.row.detail.spec.selector">
                <div v-for="(v,k) in scope.row.detail.spec.selector">{{v}}:{{item.port}} {{item.protocol}}</div>
                <div v-for="(v,k) in scope.row.detail.spec.selector" v-if="item.nodePort">{{v}}:{{item.nodePort}} {{item.protocol}}</div>
              </template>
              <template v-else>
                <div>{{scope.row.name}}:{{item.port}} {{item.protocol}}</div>
                <div v-if="item.nodePort">{{scope.row.name}}:{{item.nodePort}} {{item.protocol}}</div>
              </template>

            </div>
          </template>
        </el-table-column>
        <el-table-column sortable label="外部端点" prop="">
          <template slot-scope="scope">
<!--            <template v-if="scope.row.type === 'LoadBalancer'">-->
              <div v-for="(item, key) in scope.row.detail.spec.ports" :key="key">
                <div v-for="(v,k) in scope.row.detail.status.loadBalancer.ingress">{{v.ip}}:{{item.port}}</div>
              </div>
<!--            </template>-->
          </template>
        </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        dialogNewTgt: false,
        tableData: [],
        url: this.GLOBAL.service_tag+'/api/kubernetes/service/',
