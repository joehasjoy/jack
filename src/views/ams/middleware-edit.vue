		...
		>
			...
			<div v-show="showStepStaff">
				<el-form-item
					...
					prop="radio"
				>
					<el-radio v-model="form.isCore" label="1" @change="form.scopeLevel = ''">是</el-radio>
					<el-radio v-model="form.isCore" label="2" @change="form.scopeLevel = ''">否</el-radio>
				</el-form-item>
				...
				>
					...
					<el-radio v-model="form.scopeLevel" label="2" @change="form.includeCodes = []">业务域</el-radio>
					<el-radio v-model="form.scopeLevel" label="3" @change="form.includeCodes = []">应用</el-radio>
				</el-form-item>
...
export default {
	...
	methods: {
		...
		queryDataById() {
			let url = `${this.GLOBAL.baseUrl}/middleware/info/${this.$route.query.id}`;
			this.axios({
				method: "get",
				url: url,
			})
				.then((res) => {
					if (res.status === 200) {
						...
						response.isCore = '' + response.isCore
						response.scopeLevel = response.scopeLevel ? '' + response.scopeLevel : ''
						response.includeCodes = response.includeCodes ? response.includeCodes.split(',') : []
